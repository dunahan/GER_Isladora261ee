...
//-- the wing/tail 2das can't go above 255 anyway. change these to reflect the max number of whatever
//-- that you have in your module. (or that the user may have in their override) ie: you may not have 200 female human heads, though it is possible.
int WINGMAX     =   255;
int TAILMAX     =   255;
...
int ALLOWBONEREMOVAL=   0;  //-- palemasters can remove bone arms
int ALLOWRDDWING    =   0;  //-- anybody can get red dragon wings
int ALLOWRDDREMOVAL =   0;  //-- red dragon disciples can remove wings
...
/* instructions:
   you can set individual wings/tails or ranges of them to be un-selectable in the body shop tailor.
   for example, if you dont want people to have angel or demon wings, set a case # for those, like this:
   case CREATURE_WING_TYPE_ANGEL:
   case CREATURE_WING_TYPE_DEMON:
     return TRUE;

   you can also use straight numbers, which match the 2da line numbers. ie: demon = 1 and angel = 2. custom wings will use plain numbers.
   the scripts should skip blank 2da lines, but to make this process run more smoothly, you should indicate any large gaps in your 2da files,
   using the invalid range sets, thus:
     if(n >= 43 && n <= 93)  return TRUE;

   this will skip lines 43-93.

   if you touch nothing, there won't be any restrictions. you can still use the soft max constants to tell the scripts
