...
void main()
{
  ...
  //SetLocalObject(OBJECT_SELF, "tlr_Client", GetPCSpeaker());
    ...
    string s2DAend, sNewApp; int iNewApp;
    //-- this is set by the tlr_onconv script, which has to go in the creature conv slot
    if (GetLocalObject(OBJECT_SELF, "tlr_Client") == oPC)
    {
        if (GetListenPatternNumber() == 8888)   sNewApp = GetMatchedSubstring(0);
        iNewApp = StringToInt(sNewApp);
        if (iNewApp < 0)    iNewApp = 1;
    }
...
//--WING SECTION vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
    ...
    {
        if (s2DAend == "FAIL" || iNewApp > WINGMAX) iNewApp = 0;
        else    iNewApp++;
