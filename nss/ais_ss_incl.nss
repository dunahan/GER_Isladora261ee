...
void AISS_WanderingMonsters(object oArea = OBJECT_SELF)
{
    ...
    DebugMode("AISS_WanderingMonsters: found "+IntToString(iMobGroups)+" mob groups");
    for(i=1;i<=iMobGroups;i++)
        DebugMode("AISS_WanderingMonsters: Group "+IntToString(i)+" resref: "+
...
void AISS_RandomPlaceables(object oArea = OBJECT_SELF)
{
    ...
    DebugMode("AISS_RandomPlaceables: found "+IntToString(iPlacGroups)+" placeables groups");
    for(i=1;i<=iPlacGroups;i++)
        DebugMode("AISS_RandomPlaceables: Group "+IntToString(i)+" resref: "+
...
void AISS_FixedPlaceablesGroup(object oArea = OBJECT_SELF)
{
     ...
     while (GetIsObjectValid(obj))
     {
            DebugMode("AISS_FixedPlaceablesGroup: found placeable waypoint");
            //find spawn composition
            while (GetLocalString(obj,"sAISS_plac_resref_group_"+IntToString(i))!="")
                i++;
            iPlacGroups = i-1;

            DebugMode("AISS_RandomPlaceables: found "+IntToString(iPlacGroups)+" placeables in waypoint");
...
void AISS_RandomPlacedTraps(object oArea = OBJECT_SELF)
{
    ...
    DebugMode("AISS_RandomPlacedTraps: found "+IntToString(iTrapGroups)+" trap groups");
    ...
    for(i=1;i<=iTrapGroups;i++)
    {
        ...
        {
            DebugMode("AISS_RandomPlacedTraps: placing "+
                            GetLocalString(oArea,"sAISS_trap_group_type_"+IntToString(i))+" "+
...
void AISSS_ClearArea(object oArea = OBJECT_SELF)
{
    ...
    DebugMode("AISSS_ClearArea launched!");
    int i = 1, j = 0;
    ...
    DebugMode("AISSS_ClearArea: area reset successfully!");
}
