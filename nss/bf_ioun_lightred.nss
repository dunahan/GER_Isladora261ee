...
Ioun Stone - Light Light Red (bf_ioun_lightred)

+1 to Dexterity as an Extraordinary Effect, ie only removed by resting.
...
void main()
{
    ...
    if (GetLocalInt(GetItemActivator(), "nCount") <= 2)
    {
        if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
        {
            ...
            effect eEffect = EffectAbilityIncrease(ABILITY_DEXTERITY, 1);

            effect eVis = EffectVisualEffect(7169);
            DebugMode("Iounstone Light Red - VFX 7169");
