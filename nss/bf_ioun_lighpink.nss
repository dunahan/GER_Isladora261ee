...
Ioun Stone - Light Pink (bf_ioun_lighpink)

+1 to Intelligence as an Extraordinary Effect, ie only removed by resting.
...
void main()
{
    ...
    if (GetLocalInt(GetItemActivator(), "nCount") <= 2)
    {
        if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
        {
            ...
            effect eEffect = EffectAbilityIncrease(ABILITY_INTELLIGENCE, 1);

            effect eVis = EffectVisualEffect(7169);
            DebugMode("Iounstone Light Pink - VFX 7169");
