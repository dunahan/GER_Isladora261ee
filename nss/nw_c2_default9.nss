...
//NEW lines for setup of real dead looking Undead.
    string sUndead = GetResRef(OBJECT_SELF);

    // Skelette
    if (//sUndead == "NW_SKELETON"    || sUndead == "NW_SKELCHIEF"    || sUndead == "NW_SKELMAGE" ||
        //sUndead == "NW_SKELPRIEST"  || sUndead == "NW_SKELWARR01"   || sUndead == "NW_SKELWARR02" ||
        //sUndead == "mob_knochw_001" || sUndead == "mob_niedw_001"   || sUndead == "mob_grabw_001" ||
        //sUndead == "mob_elitew_001" || sUndead == "mob_skelhex_001" || sUndead == "mob_verwmag_001" ||
        sUndead == "mob_skelett_n"    || sUndead == "mob_skelett_m" )
    {
        SetIsDestroyable       (FALSE, TRUE, FALSE);
        ChangeToStandardFaction(OBJECT_SELF, STANDARD_FACTION_COMMONER);
        ApplyEffectToObject    (DURATION_TYPE_INSTANT, EffectDeath(), OBJECT_SELF);
        object oUndeadIO = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_undead",
                                        GetLocation(OBJECT_SELF));
        SetLocalString         (oUndeadIO, "UndeadTag", GetTag(OBJECT_SELF));
    }

    // Zombies
    if (//sUndead == "NW_ZOMBTYRANT" || sUndead == "NW_ZOMBIE01"   || sUndead == "NW_ZOMBIE02" ||
        //sUndead == "NW_ZOMBIEBOSS" || sUndead == "NW_ZOMBWARR01" || sUndead == "NW_ZOMBWARR02" ||
        sUndead == "mob_zombie_a"    || sUndead == "mob_zombie_b" )
    {
        SetIsDestroyable       (FALSE, TRUE, FALSE);
        ChangeToStandardFaction(OBJECT_SELF, STANDARD_FACTION_COMMONER);
        ApplyEffectToObject    (DURATION_TYPE_INSTANT, EffectDeath(), OBJECT_SELF);
    ...
    // Added flyby support - these creatures will now use the flyby AI script.
    // These are the correct tag, the spelling error is BioWare's  ;-)
    // You may also wish to do the same for default NWN dragons.
    string sTag = GetTag(OBJECT_SELF);
    if(sTag == "NW_DMSUCUBUS"    || sTag == "NW_S_SUCCUBUS"   ||
       sTag == "x2_erinyes"      ||
       // Dragon Wyrmling
       sTag == "x0_wyrmling_blk" || sTag == "x0_wyrmling_blu" ||
       sTag == "x0_wyrmling_brs" || sTag == "x0_wyrmling_brz" ||
       sTag == "x0_wyrmling_cop" || sTag == "x0_wyrmling_gld" ||
       sTag == "x0_wyrmling_grn" || sTag == "x0_wyrmling_red" ||
       sTag == "x0_wyrmling_sil" || sTag == "x0_wyrmling_wht"   )
    {
        SetCreatureOverrideAIScript(OBJECT_SELF, "bd_ai_flyby");
        SetLocalInt(OBJECT_SELF, "FlybyInfrequency", 2);
    }

    // Added Dragon Combat support - these creatures will now use the Dragon Combat
    // AI script.
    string sDrag = GetTag(OBJECT_SELF);
    if(// Adult
       sDrag == "nw_drgblack001" || sDrag == "nw_drgblue001"  ||
       sDrag == "nw_drgbrass001" || sDrag == "nw_drgbrnz001"  ||
       sDrag == "nw_drgcopp001"  || sDrag == "nw_drggold001"  ||
       sDrag == "nw_drggreen001" || sDrag == "nw_drgred001"   ||
       sDrag == "nw_drgsilv001"  || sDrag == "nw_drgwhite001" ||
       // Old
       sDrag == "nw_drgblack002" || sDrag == "nw_drgblue002"  ||
       sDrag == "nw_drgbrass002" || sDrag == "nw_drgbrnz002"  ||
       sDrag == "nw_drgcopp002"  || sDrag == "nw_drggold002"  ||
       sDrag == "nw_drggreen002" || sDrag == "nw_drgred002"   ||
       sDrag == "nw_drgsilv002"  || sDrag == "nw_drgwhite002" ||
       // Ancient
       sDrag == "nw_drgblack003" || sDrag == "nw_drgblue003"  ||
       sDrag == "nw_drgbrass003" || sDrag == "nw_drgbrnz003"  ||
       sDrag == "nw_drgcopp003"  || sDrag == "nw_drggold003"  ||
       sDrag == "nw_drggreen003" || sDrag == "nw_drgred003"   ||
       sDrag == "nw_drgsilv003"  || sDrag == "nw_drgwhite003" ||
       // New Dragons
       sDrag == "x2_dragonshad001"
       )
    {
        SetCreatureOverrideAIScript(OBJECT_SELF, "bd_ai_dragon");
        SetLocalInt(OBJECT_SELF, "FlybyStompPower", GetCreatureSize(OBJECT_SELF));
        SetLocalInt(OBJECT_SELF, "FlybyInfrequency", 2);
    }
    ...
    if (GetStandardFactionReputation(STANDARD_FACTION_COMMONER, OBJECT_SELF) <= 10 &&
        GetLocalInt(OBJECT_SELF, "GotIt") < 1 )
    {
      if (HasItem(OBJECT_SELF, "NW_IT_GOLD001") == FALSE)
      {
        string sBoni = TreaGetAreaBonus(GetResRef(GetArea(OBJECT_SELF)));       // bringt "BossBonus|0|NormBonus|0|BossChance|25|NormChance|35|"
        TreaGetTreasure(TreaGetTreasureTable(OBJECT_SELF), OBJECT_SELF, StringToInt(GetTokenFromString(8, sBoni)), StringToInt(GetTokenFromString(4, sBoni)));
        SetLocalInt(OBJECT_SELF, "GotIt", 1);
      }
    }

    if (GetResRef(OBJECT_SELF) == "mob_ratte_n" ||
        GetResRef(OBJECT_SELF) == "mob_ratte_boss" ||
        GetResRef(OBJECT_SELF) == "cre_eber_n"
        )
    {
      DelayCommand(5.9, DeleteUnwantedItemsByChance("hw_resleath"));              // wg. lederratten....
      DelayCommand(6.0, DeleteUnwantedItemsByChance("cnranimalmeat"));            // wg. fleischeber usw
    }
}
