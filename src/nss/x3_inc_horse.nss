...
// single blueprint.
object HorseCreateHorse(string sResRef,location lLoc,object oOwner=OBJECT_INVALID,string sTag="",int nAppearance=-1,int nTail=-1,int nFootstep=-1,string sScript="",string sName="");
...
{ // PURPOSE: Spawn a horse with the specified information
    ...
    int bCustomName=GetStringLength(sName) > 0;
    ...
    { // resref specified
        ...
        { // horse successfully created
            ...
            if (bCustomName)
            {
                SetLocalString(oHorse,"sX3_CustomName", sName);
            ...
            { // assign owner
                ...
                if (!bCustomName && GetMaster(oHorse)==oOwner) SetName(oHorse,"Pferd von "+GetName(oOwner));
...
{ // PURPOSE: Set oHorse to be owned by oOwner
    ...
    { // valid parameters
        ...
        { // horse can be mounted
            ...
            sCustomName=GetLocalString(oHorse,"sX3_CustomName");
            bCustomName=GetStringLength(sCustomName) > 0;
            if (bCustomName)
            {
                SetName(oHorse, sCustomName);
            }
            if (GetMaster(oHorse)==oOwner)
            { // was set okay
                if (!bCustomName) SetName(oHorse,"Pferd von "+GetName(oOwner));
            } // was set okay
        } // horse can be mounted
        else
        { // not valid
...
{ // PURPOSE: Remove the owner from oHorse
    ...
    if (GetStringLength(GetLocalString(oHorse,"sX3_CustomName"))<1)
    {
  ...
  // 6 = destroy oHorse
    ...
    { // main mounting switch
        ...
        { // ----- 4 - Store Horse and Premount Info
            ...
            SetLocalString(oRider,"sX3_CustomName",GetLocalString(oHorse,"sX3_CustomName"));
    ...
    { // oRider is a valid object
        ...
        { // Mounted
            ...
            { // create dismounted horse
                ...
                sCN=GetLocalString(oRider,"sX3_CustomName");
                ...
                oHorse=HorseCreateHorse(sRR,GetLocation(oRider),oOb,sT,nApp,nTail,nFootstep,sS,sCN);
                SetLocalInt(oHorse,"X3_DOING_HORSE_ACTION",TRUE);
...
{ // PURPOSE: To store inventory being carried by oCreature
    ...
    { // do not use database
        ...
        SetName(oChest,GetName(oCreature)+"s Inventar");
...
{ // PURPOSE: HorseChangeToDefault
    int nTail = GetDefaultTail(oCreature); // Standardwert: CREATURE_TAIL_TYPE_NONE
    int nDefApp = GetDefaultAppearance(oCreature);
    int nDefPhen;
    int nPheno=GetPhenoType(oCreature);
    if (GetSubRace(oCreature) == "-1") return;  // Brich ab, da es eine Sonderart ist
...
{ // PURPOSE: See if not default appearance
    int nDefApp = GetDefaultAppearance(oCreature);
    int nPheno = GetPhenoType(oCreature);
    if (GetAppearanceType(oCreature) != nDefApp ||
        nPheno == HORSE_PHENOTYPE_JOUSTING_L ||
        nPheno == HORSE_PHENOTYPE_JOUSTING_N ||
        nPheno == HORSE_PHENOTYPE_MOUNTED_L ||
        nPheno == HORSE_PHENOTYPE_MOUNTED_N) HorseChangeToDefault(oCreature);
