...
void main()
{
    ...
    // string sModName = GetName(GetModule());                                  // switched to const string!

    // End script if any of these conditions are met
    if (!GetIsPC(oPC) ||
       //GetIsDM(oPC) ||                                                        // DM's should also possible
    ...
    if (sUserID != "" && sUserID != sID)
    {
      ...
      //AssignCommand(oPC, ActionMoveAwayFromObject(oChest, TRUE, 50.0));
        FloatingTextStringOnCreature("Diese Truhe gehört nicht Dir!", oPC);
