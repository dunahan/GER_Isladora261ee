...
Ioun Stone - Light Yellow (bf_ioun_ligyello)

+1 to Charisma as an Extraordinary Effect, ie only removed by resting.
...
void main()
{
    ...
    if (GetLocalInt(GetItemActivator(), "nCount") <= 2)
    {
        if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
        {
            ...
            effect eEffect = EffectAbilityIncrease(ABILITY_CHARISMA, 1);

            effect eVis = EffectVisualEffect(7172);
            DebugMode("Iounstone Light Yellow - VFX 7172");
