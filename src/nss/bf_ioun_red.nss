...
Ioun Stone - Red (bf_ioun_red)

+2 to Dexterity as an Extraordinary Effect, ie only removed by resting.
...
void main()
{
    ...
    if (GetLocalInt(GetItemActivator(), "nCount") <= 2)
    {
        if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
        {
            ...
            effect eEffect = EffectAbilityIncrease(ABILITY_DEXTERITY, 2);

            effect eVis = EffectVisualEffect(7169);
            DebugMode("Iounstone Red - VFX 7169");
