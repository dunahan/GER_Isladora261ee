...
Ioun Stone - White (bf_ioun_white)

Regenerate 1hp every 6 seconds. as an Extraordinary Effect, ie only removed by resting.
...
void main()
{
    ...
    if (GetLocalInt(GetItemActivator(), "nCount") <= 2)
    {
        if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
        {
            ...
            effect eEffect = EffectRegenerate(1, 6.0);
            effect eVis = EffectVisualEffect(7173);
            DebugMode("Iounstone White - VFX 7173");
