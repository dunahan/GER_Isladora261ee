...
/////////////////////////////////////////////////////////      was missing in cnr 305?
void CnrKillAllTradeskillXPs(object oPC)
{
  int i;
  for (i=0; i<=11; i++)
  {
    CnrSetPersistentInt(oPC, "CnrTradeskillXP_"+IntToString(i), 0); // old CNR Style
    CnrUpdateTopTenCrafters(oPC, i, 0);
  }

  CnrSetPersistentInt(oPC, "CnrTradeskillXP_"+IntToString(101), 0); // old CNR Style
  CnrUpdateTopTenCrafters(oPC, 101, 0);
}
  ...
  if (sRecipeBiTag != "BIPRODUCT_INVALID")
  {
    ...
    if (nItemCount > 0)
    {
      ...
      for (nBatch=0; nBatch<(nBatchCount*nItemCount); nBatch++)
      {
        ...
        {
          CnrRecipeCreateItemOnObject(sRecipeBiTag, oDevice, 1);
        }
        else
        {
  ...
  if (bSuccess)
  {
    ...
    for (nBatch=0; nBatch<nBatchCount; nBatch++)
    {
      ...
      for (nItem=0; nItem<nRecipeQty; nItem++)
      {
        if (bSpawnItemInDevice)
        {
          CnrRecipeCreateItemOnObject(sRecipeTag, oDevice, 1);
        }
        else
        {
    ...
    if (nScaledGameXP > 0)
    {
      sInfo1 = CNR_TEXT_YOUR_ADVENTURING_XP_INCREASED_BY + IntToString(nScaledGameXP*nBatchCount) + CNR_TEXT_XP_IN_DECREASED_END + "\n";
    }
    else if (nScaledGameXP < 0)
    {
      sInfo1 = CNR_TEXT_YOUR_ADVENTURING_XP_DECREASED_BY + IntToString(nScaledGameXP*nBatchCount) + CNR_TEXT_XP_IN_DECREASED_END + ".\n";
    }
    ...
    if (nDeviceTradeskillType != CNR_TRADESKILL_NONE)
    {
      ...
      if (nScaledTradeXP > 0)
      {
        sInfo2 = CNR_TEXT_YOUR + sTradeName + CNR_TEXT_XP_INCREASED_BY + IntToString(nScaledTradeXP*nBatchCount) + CNR_TEXT_XP_IN_DECREASED_END + "\n\n";
        bUpdateJournal = TRUE;
      }
      else if (nScaledTradeXP < 0)
      {
        sInfo2 = CNR_TEXT_YOUR + sTradeName + CNR_TEXT_XP_DECREASED_BY + IntToString(nScaledTradeXP*nBatchCount) + CNR_TEXT_XP_IN_DECREASED_END + "\n\n";
        bUpdateJournal = TRUE;
      }
      ...
      if (bUpdateJournal)
      {
        ...
        if (!bLevelDenied)
        {
          if (nNewLevel < 20)
          {
            ...
            nLevelXP -= nNewXP;
            sInfo2 += CNR_TEXT_YOU_NEED + IntToString(nLevelXP) + CNR_TEXT_XP_TO_REACH_THE_NEXT_LEVEL_IN + sTradeName + "." + CNR_TEXT_XP_TO_REACH_THE_NEXT_LEVEL_IN2;
          }
    ...
    sResult = CNR_TEXT_YOU_SUCCESSFULLY_MADE + IntToString(nBatchCount*nRecipeQty);
    if ((nBatchCount*nRecipeQty) > 1)
    {
      sResult += CNR_TEXT_ITEMS;
    }
    else
    {
      sResult += CNR_TEXT_ITEM;
    }
