...
void main()
{
    ...
    {
        if(GetIsEnemy(oTarget) && oTarget != OBJECT_SELF                        // affect only enemy AND not it self OR
           || oTarget != GetMaster(OBJECT_SELF)                                 // not the master (so the master of the animal shouldn't be affected)
          )
        {
