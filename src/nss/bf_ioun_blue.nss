...
Ioun Stone - Blue (bf_ioun_blue)

+2 to Wisdom as an Extraordinary Effect, ie only removed by resting.
...
void main()
{
    ...
    if (GetLocalInt(GetItemActivator(), "nCount") <= 2)
    {
        if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
        {
            ...
            effect eEffect = EffectAbilityIncrease(ABILITY_WISDOM, 2);

            effect    eVis = EffectVisualEffect(7168);
            DebugMode("Iounstone Blue - VFX 7168");
