...
const float GP_REWARD_MULTIPLIER = 0.1;
...
const float PARTY_XP_GP_BONUS = 0.2;
...
// Displays XP status info after each kill
...
const int
LEVEL_1_MAX_XP  = 100,    LEVEL_11_MAX_XP = 100,
LEVEL_2_MAX_XP  = 100,    LEVEL_12_MAX_XP = 100,
LEVEL_3_MAX_XP  = 100,    LEVEL_13_MAX_XP = 100,
LEVEL_4_MAX_XP  = 100,    LEVEL_14_MAX_XP = 100,
LEVEL_5_MAX_XP  = 100,    LEVEL_15_MAX_XP = 100,
LEVEL_6_MAX_XP  = 100,    LEVEL_16_MAX_XP = 100,
LEVEL_7_MAX_XP  = 100,    LEVEL_17_MAX_XP = 100,
LEVEL_8_MAX_XP  = 100,    LEVEL_18_MAX_XP = 100,
LEVEL_9_MAX_XP  = 100,    LEVEL_19_MAX_XP = 100,
LEVEL_10_MAX_XP = 100,    LEVEL_20_MAX_XP = 100,

LEVEL_21_MAX_XP = 100,    LEVEL_31_MAX_XP = 100,
LEVEL_22_MAX_XP = 100,    LEVEL_33_MAX_XP = 100,
LEVEL_23_MAX_XP = 100,    LEVEL_32_MAX_XP = 100,
LEVEL_24_MAX_XP = 100,    LEVEL_34_MAX_XP = 100,
LEVEL_25_MAX_XP = 100,    LEVEL_35_MAX_XP = 100,
LEVEL_26_MAX_XP = 100,    LEVEL_36_MAX_XP = 100,
LEVEL_27_MAX_XP = 100,    LEVEL_37_MAX_XP = 100,
LEVEL_28_MAX_XP = 100,    LEVEL_38_MAX_XP = 100,
LEVEL_29_MAX_XP = 100,    LEVEL_39_MAX_XP = 100,
LEVEL_30_MAX_XP = 100,    LEVEL_40_MAX_XP = 100; // No need for this really
...
void main()
{
    ...
    // Item suchen
    object oItem = GetItemPossessedBy(oKiller, "itm_questbuch");
    // SC hat XP- Strafe! Am Anfang des XP Skripts, keine Ausführung dessen notwendig!
    if (GetLocalInt(oItem, "Restricted") == 1)
    { WriteTimestampedLogEntry(GetName(oKiller) + " hat eine XP Strafe erhalten und deswegen keine XP bekommen.");
      return; }
    ...
    // Added ECL for Subraces
    if ( nECL >= 1 )
    { switch (nECL)
      {
        case 1: // Sonderrasse mit ECL 5%, zb Steingnome usw
          nXPToGive = ( (nXPToGive * 95) / 100 );
          GiveXP(oKiller, nXPToGive, fKillerBonus, nDiff, nPlayer);
          XPDebugMessage(oKiller, fCR, nDiff, nLoLevel, nHiLevel, fAvLevel);
        break; // Abzug von 5% EP, nXP gegen Variable tauschen!

        case 2: // Sonderrasse mit ECL 10%, zb Drow, Genasi
          nXPToGive = ( (nXPToGive * 90) / 100 );
          GiveXP(oKiller, nXPToGive, fKillerBonus, nDiff, nPlayer);
          XPDebugMessage(oKiller, fCR, nDiff, nLoLevel, nHiLevel, fAvLevel);
        break; // Abzug von 10% EP

        case 3: // Sonderrasse mit ECL 15%, Monsterrassen!
          nXPToGive = ( (nXPToGive * 85) / 100 );
          GiveXP(oKiller, nXPToGive, fKillerBonus, nDiff, nPlayer);
          XPDebugMessage(oKiller, fCR, nDiff, nLoLevel, nHiLevel, fAvLevel);
        break; // Abzug von 15% EP
      }
    }
        ...
        if (GetIsPC(oParty))
        {
            if (!GetIsDead(oParty) && fDist >= 0.0 && fDist <= PARTY_DIST)
            {
                ...
                if (oKiller == oParty && nPlayer > 1)
                {
                    ...
                    {
                        FloatingTextStringOnCreature(ColorString("Der Levelunterschied ist zu gross.",255,0,0), oParty);
                        nXPToGive = MIN_XP;
                    }
                    if (FLOATING_TEXT)
                    {
                    ...
                    SetXPImproved(oParty, GetXP(oParty)+(nXPToGive + FloatToInt(fKillerBonus)));
                }
                ...
                else
                {
                    ...
                    SetXPImproved(oParty, GetXP(oParty)+nXPToGive);
                }
            }
        }
